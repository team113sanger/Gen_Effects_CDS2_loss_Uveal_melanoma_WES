...
option_list<- list(
  ...
  make_option(c("--rg_id" ), action="store_true",type="character", default=NA, help="Read group ID for the lane "),
...
parser<- OptionParser(usage = paste(progname, " [options] --sample_name PDXXXXXa --human_bam  /PATH/TO/PDXXXXa.human.bam --mouse_bam  /PATH/TO/PDXXXXa.mouse.bam --rg_id RUN_LANE_TAG --outdir /PATH/TO/OUTPUT --ncpu 1 ",sep=""), option_list=option_list)
...
if(is.na(opt$outdir)){
  ...
  outdir<-file.path(opt$outdir, paste0(opt$sample_name, "_",  opt$rg_id))
}
if(!dir.exists(outdir)){ # Create the outdir if it doesn't exist
  dir.create(outdir, recursive = T )
}
#Exit if the read group is not present
if(is.na(opt$rg_id)){
  stop(paste("No read group provided please provide the read group name -required_parameter-", sep=""))
}
...
outfname<-paste0(sample_name,"_", opt$rg_id, ".sample.dupmarked.mXfilt.bam")
